...
export async function POST(req: Request) {
  ...
  // Category sent to SABnzbd. The client's configured category is preferred
  // because it is guaranteed to exist in SABnzbd (so the download is sorted
  // correctly); fall back to the release category, then a sane default.
  ...
  try {
    ...
    // Try to recover an NZB/archive password embedded in the NZB metadata or title.
...
/**
 * Pull live status from the download client for any still-active grabs and
 * persist the result. Without this, grabs stay "queued" forever and the Stats
 * page shows everything as 0.
 */
function grabNeedsSabSync(g: Grab): boolean {
  ...
  // One-time backfill of SAB status/warnings on terminal grabs.
...
export async function GET(req: Request) {
  ...
  // Sync live download status before returning so progress / completion
  // (and therefore the Stats page) reflect reality.
  const synced = await syncActiveGrabs(grabList);

  // Attach usernames for the admin view (no Drizzle relations are defined)
