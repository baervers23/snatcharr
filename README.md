...
<p align="center">
  ...
  <a href="https://hub.docker.com/r/baervers23/snatcharr"><img src="https://img.shields.io/docker/image-size/baervers23/snatcharr/0.9.0-beta?style=flat-square" alt="Docker image size"></a>
...
**v0.9.0-beta** — A self-hosted Usenet search and grab manager for home labs and Jellyfin setups.
...
The compose file uses **`baervers23/snatcharr:0.9.0-beta`** by default.
...
2. Set `NEXTAUTH_URL` / `AUTH_URL` to your public HTTPS URL.
...
```bash
docker build -t baervers23/snatcharr:0.9.0-beta --build-arg APP_VERSION=0.9.0-beta .
```
...
docker run -d \
  ...
  baervers23/snatcharr:0.9.0-beta
```

Push to a registry (after `docker login`):

```bash
docker push baervers23/snatcharr:0.9.0-beta
```
