...
export async function logAuditAction(input: ActionLogInput & { auditKey?: string; userId?: string }): Promise<void> {
  logAction(input);
...
/** Legacy helper — writes raw line (avoid for new code). */
export function logRaw(level: LogLevel, line: string): void {
