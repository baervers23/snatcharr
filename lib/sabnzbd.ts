...
/** Still in SAB post-processing (extract/move/etc.) — not ready to download yet. */
export function sabHistoryIsProcessing(item: SabnzbdHistoryItem): boolean {
  return !sabHistoryIsFailed(item) && !sabHistoryIsCompleted(item);
}
...
export async function getSabnzbdJobStatus(
  ...
  if (queueItem) {
    ...
    if (sabQueueIsPostProcessing(queueItem)) {
      return { status: "processing", progress: 1, speedBps: 0 };
    }
    return {
      status: "downloading",
      progress,
  ...
  if (histItem) {
    if (sabHistoryIsFailed(histItem)) {
      return { status: "failed", progress: 0, speedBps: 0 };
    }
    if (sabHistoryIsProcessing(histItem)) {
      return {
        status: "processing",
        progress: 1,
        speedBps: 0,
